...
function apiGetMovies() {
  fetch(`${url}//search/movies/intheatres`).then(r => {
    return r.json();
  }).then(r => allMovies = r.results).then(r => populateIndexAndScroll(r));
}
...
function indexMovies(movies) {
  ...
  movies.map(movie => {
    ...
    <div data-id="${movie.id}" id="movie-card-${movie.id}" class="movie-card">
       <img data-id="${movie.id}" class="movie-image hvr-grow"  src="${imagePath + movie.poster_path}" alt="">
        <h4 class="movie-title" data-id="${movie.id}">${movie.title}</h4>
      </div>
    `;
  });
...
const getVideos = id => {
  fetch(`${url}/search/videos/${id}`).then(r => r.json())
  // .then(r => console.log(r.results[0].key));
  .then(r => movieShow.innerHTML += `
        <div class="trailer-iframe-container">
        <iframe class="trailer-iframe" src="https://www.youtube.com/embed/${r.results[0].key}" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
        </div>
        `);
};
